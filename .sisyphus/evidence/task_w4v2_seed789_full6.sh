...
LOG=.sisyphus/evidence/task-w4v2-seed789-train.log
if [[ -f "$LOG" ]]; then
...
on_exit() {
  rc=$?
  echo "=== w4v2 seed789 full6 real-coz grpo end $(date -Is) rc=${rc} ==="
  exit "$rc"
}
trap on_exit EXIT

echo "=== w4v2 seed789 full6 real-coz grpo start $(date -Is) ==="
hostname
...
# R_rep + R_fb headline reward. The reward GPU is intentionally shared with the
# 6-GPU FSDP/vLLM pool instead of being held out as a mostly idle dedicated GPU.
export COZ_ENABLE_RFB=1
export COZ_REWARD_DEVICE=cuda:5
export COZ_REWARD_SR_DEVICE=cuda:5
export COZ_REWARD_IQA_DEVICE=cuda:5
export COZ_REWARD_CLIP_DEVICE=cuda:5
...
python .sisyphus/evidence/verl_direct_controller.py \
  ...
  actor_rollout_ref.rollout.seed=789 \
  actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu=1 \
  actor_rollout_ref.rollout.max_num_batched_tokens=4096 \
  actor_rollout_ref.rollout.max_model_len=4096 \
  actor_rollout_ref.rollout.max_num_seqs=36 \
  ...
  trainer.default_local_dir=checkpoints/w4v2_seed789/coz_grpo \
  trainer.resume_mode=disable \
  trainer.max_actor_ckpt_to_keep=4 \
  'trainer.logger=["console"]' \
  trainer.project_name=w4v2_seed789 \
  trainer.experiment_name=coz_grpo_rrep_rfb_seed789_full6 \
  ray_init.num_cpus=8
