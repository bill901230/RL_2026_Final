...
on_exit() {
  rc=$?
  echo "=== w4v2 real-coz grpo end $(date -Is) rc=${rc} ==="
  exit "$rc"
}
trap on_exit EXIT

echo "=== w4v2 real-coz grpo start $(date -Is) ==="

source .venv-train/bin/activate

# Four GPUs for veRL workers; cuda:5 remains visible and dedicated to R_fb.
export CUDA_VISIBLE_DEVICES=0,1,2,3,4,5
unset ROCR_VISIBLE_DEVICES HIP_VISIBLE_DEVICES
export RAY_TMPDIR=/tmp/$USER/ray-w4v2
export TMPDIR=/tmp/$USER/tmp-w4v2
...
# Reward design: R_rep + R_fb carry the signal, anchor kept deliberately small.
...
python .sisyphus/evidence/verl_direct_controller.py \
  ...
  trainer.default_local_dir=checkpoints/w4v2/coz_grpo \
  trainer.resume_mode=disable \
  trainer.max_actor_ckpt_to_keep=4 \
  'trainer.logger=["console"]' \
  trainer.project_name=w4v2 \
  trainer.experiment_name=coz_grpo_rrep_rfb \
  ray_init.num_cpus=8
