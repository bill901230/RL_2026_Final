...
LOG=.sisyphus/evidence/task-w4-train.log
exec > >(tee "$LOG") 2>&1

on_exit() {
  rc=$?
  echo "=== w4 real-coz grpo end $(date -Is) rc=${rc} ==="
  exit "$rc"
}
trap on_exit EXIT

echo "=== w4 real-coz grpo start $(date -Is) ==="
...
python .sisyphus/evidence/verl_direct_controller.py \
  ...
  trainer.default_local_dir=checkpoints/w4/coz_a6_verl \
  trainer.resume_mode=disable \
  trainer.max_actor_ckpt_to_keep=2 \
  'trainer.logger=["console"]' \
  trainer.project_name=w4 \
  trainer.experiment_name=coz_a6_verl \
  ray_init.num_cpus=8
