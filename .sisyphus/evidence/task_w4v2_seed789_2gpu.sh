...
on_exit() {
  rc=$?
  echo "=== w4v2 seed789 2gpu real-coz grpo end $(date -Is) rc=${rc} ==="
  exit "$rc"
}
trap on_exit EXIT

echo "=== w4v2 seed789 2gpu real-coz grpo start $(date -Is) ==="
hostname
...
# R_rep + R_fb headline reward. With the current 2-GPU allocation the SR reward
# shares visible cuda:1 with training; H200 memory headroom is ample.
...
python .sisyphus/evidence/verl_direct_controller.py \
  ...
  trainer.experiment_name=coz_grpo_rrep_rfb_seed789_2gpu \
  ray_init.num_cpus=8
