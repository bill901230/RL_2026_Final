...
CHECKPOINT_DIR="${COZ_CHECKPOINT_DIR:-checkpoints/A7_stable}"
LOG="${COZ_LOG:-.sisyphus/evidence/task-A7-stable-train.log}"
if [[ -f "$LOG" ]]; then
...
on_exit() {
  rc=$?
  echo "=== A7 stable ${RUN_TAG} seed123 2gpu real-coz grpo end $(date -Is) rc=${rc} ==="
  exit "$rc"
}
trap on_exit EXIT

echo "=== A7 stable ${RUN_TAG} seed123 2gpu real-coz grpo start $(date -Is) ==="
hostname
...
python .sisyphus/evidence/verl_direct_controller.py \
  ...
  trainer.project_name=A7_stable \
  trainer.experiment_name="coz_grpo_stateexp_seed123_2gpu_${RUN_TAG}" \
  ray_init.num_cpus=8
