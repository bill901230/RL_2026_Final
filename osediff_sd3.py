...
class StableDiffusion3Base():
    def __init__(self, model_key:str='stabilityai/stable-diffusion-3-medium-diffusers', device='cuda', dtype=torch.float16):
        ...
        self.vae = pipe.vae  # keep on CPU; caller moves to target device

        self.transformer = pipe.transformer  # keep on CPU; caller moves to target device
...
class OSEDiff_SD3_TEST(torch.nn.Module):
    ...
    def forward(self, x_src, prompt):
        model_dtype = self.model.dtype  # float16, matches training precision

        z_src = self.model.vae.encode(x_src.to(dtype=model_dtype, device=self.model.vae.device)).latent_dist.sample() * self.model.vae.config.scaling_factor
        ...
        prompt_embeds = prompt_embeds.to(self.model.transformer.device, dtype=model_dtype)
        pooled_embeds = pooled_embeds.to(self.model.transformer.device, dtype=model_dtype)
        pred_v = self.predict_vector(z_src, timestep, prompt_embeds, pooled_embeds)

        z_src = z_src - pred_v

        with torch.no_grad():
            output_image = self.model.decode(z_src.to(dtype=model_dtype, device=self.model.vae.device))
